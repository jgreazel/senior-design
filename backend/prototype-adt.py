...
def findScenarios(node):
    ...
    if node["key"][0] == "L":  # If leaf node
        ...
        if len(defense) > 0:
            defenseList.append(Scenario(defense[0]["riskIndex"], defense[0]["impact"], [defense[0]["key"]]))
        ...
        for i in range(1, len(childDefenseLists)):  # Compare all combinations of scenarios
